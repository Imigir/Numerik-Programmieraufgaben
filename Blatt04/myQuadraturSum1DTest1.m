...
%{
function [v]  = myQuadratur1D2(f,s,a,b,N)
  h = (b-a)/N;
  v = 0;
  for i=1:N
    Ia = (a.+(i.-1).*h);
    ...
    if strcmp(s,"Trapez")
      x = [Ia Ib];
      w = (Ib-Ia)/2*[1 1];
    elseif strcmp(s,"Simpson")
      x = [Ia (Ia+Ib)/2 Ib];
      w = (Ib-Ia)/6*[1 4 1];
    else
      printf("Second argument error! Integral will be set to -1\n")
      v = -1;
      break;
    end
    v += myQuadratur1Dsub(f,w,x);
  end
endfunction
%}
...
% ---Plotte Fehler für f---
for i = 1:1000
  %FehlerTrapezf(i) = abs((myQuadratur1D2(f,"Trapez",0,1,i)-Intf)/Intf*100);
  FehlerTrapezf(i) = abs((myQuadratur1D(f,[0.5 0.5],[0 1],0,1,i)-Intf)/Intf*100);
  %FehlerSimpsonf(i) = abs((myQuadratur1D2(f,"Simpson",0,1,i)-Intf)/Intf*100);
...
%{
% berechnung über Aufruf einer Funktion mit anderen Argumenten
printf("2)\n")
ITrapezf2 = myQuadratur1D2(f,"Trapez",0,1,2)
ISimpsonf2 = myQuadratur1D2(f,"Simpson",0,1,2)
ITrapezg2 = myQuadratur1D2(g,"Trapez",-1,1,6)
ISimpsong2 = myQuadratur1D2(g,"Simpson",-1,1,2)
%}
